...
// 公共变量
const JKD_COOKIE = process.env.JKD_COOKIE
//只用于判断，推送是sendnodify
let SEND_KEY = '';
...
async function changeFiele () {
    let content = await fs.readFileSync('./jkd.js', 'utf8')
    //content = content.replace(/let CASH = ""/, "let CASH = 10")
    await fs.writeFileSync( './jkd.js', content, 'utf8')
}
...
async function start() {
    ...
    console.log('替换变量完毕')
    // 执行
    await exec("node jkd.js >> result.txt");
    console.log('执行完毕')
    ...
    if(SEND_KEY) {
        if (content.includes("Cookie") && Time.getHours()==9 && Time.getMinutes()<=40) {
            await notify.sendNotify("聚看点-" + new Date().toLocaleDateString(), content);
            console.log("聚看点-" + content)
        }else{
            console.log("聚看点-" + content)
        }
    }else{
