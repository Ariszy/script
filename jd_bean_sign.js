/*
京豆签到,自用,可N个京东账号,IOS软件用户请使用 https://cdn.jsdelivr.net/gh/NobyDa/Script@master/JD-DailyBonus/JD_DailyBonus.js
Node.JS专用
从 github @ruicky改写而来
version v0.0.1
create by ruicky
  ...
  fs.unlink('./result.txt', function(err){ 
    if(err) { 
      ...
      if ($.isNode() && notify.SCKEY) {
        ...
        await notify.sendNotify(`[JD账户${$.index}]${UserName}京豆签到`, content);
        console.log('发送结果完毕')
      } else {
...
async function start() {
  ...
  if ($.isNode() && notify.SCKEY) {
    ...
    await notify.sendNotify("京东签到-" + new Date().toLocaleDateString(), content);
    console.log('发送结果完毕')
  }
}
